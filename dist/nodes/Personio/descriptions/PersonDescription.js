            ...
            send: {
                ...
                preSend: [
                    async function (requestOptions) {
                        ...
                        const employment = {
                            status: this.getNodeParameter('employmentStatus'),
                            employment_start_date: this.getNodeParameter('employmentStartDate'),
                            type: this.getNodeParameter('employmentType'),
                        };
                        // Get optional employment fields from additionalFields
                        const additionalFields = this.getNodeParameter('additionalFields', {});
                        if (additionalFields.position) {
                            employment.position = { title: additionalFields.position };
                        }
                        ...
                        if (additionalFields.contractEndDate) {
                            employment.contract_end_date = additionalFields.contractEndDate;
                        }
                        ...
                        if (additionalFields.costCenterIds) {
                            const ids = additionalFields.costCenterIds
                                .split(',')
                                .map((id) => id.trim())
                                .filter((id) => id);
                            if (ids.length > 0) {
                                ...
                                employment.cost_centers = ids.map((id, index) => ({
                                    id,
        ...
        routing: {
            send: {
                ...
                preSend: [
                    async function (requestOptions) {
                        const body = requestOptions.body;
                        const customAttrsParam = this.getNodeParameter('customAttributes', {});
